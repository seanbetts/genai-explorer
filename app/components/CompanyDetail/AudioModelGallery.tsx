      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div className="space-y-6">
            ...
            {/* Audio-specific features section - e.g., supported formats */}
            {selectedModel.features?.generation && (
              <div>
                <h3 className={`${headingStyles.card} mb-3`}>Audio Features</h3>
                <div className={`${containerStyles.card}`}>
                  <div className="grid grid-cols-1 gap-4">
                    {/* Audio Quality section */}
                    <div className="mb-4">
                      <h4 className="text-sm font-semibold text-cyan-400 mb-2">Quality Options</h4>
                      <div className="grid grid-cols-2 gap-2">
                        {selectedModel.features.generation.instrumental && (
                          <div className="flex items-center h-8">
                            <i className={`${iconStyles.booleanTrue} mr-2`} />
                            <span className={textStyles.body}>Instrumental Tracks</span>
                          </div>
                        )}
                        {selectedModel.features.generation.customLyrics && (
                          <div className="flex items-center h-8">
                            <i className={`${iconStyles.booleanTrue} mr-2`} />
                            <span className={textStyles.body}>Custom Lyrics</span>
                          </div>
                        )}
                        {selectedModel.features.generation.coverArt && (
                          <div className="flex items-center h-8">
                            <i className={`${iconStyles.booleanTrue} mr-2`} />
                            <span className={textStyles.body}>Cover Art Generation</span>
                          </div>
                        )}
                        {selectedModel.features.generation.coverVideo && (
                          <div className="flex items-center h-8">
                            <i className={`${iconStyles.booleanTrue} mr-2`} />
                            <span className={textStyles.body}>Cover Video Generation</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      <h4 className="text-sm font-semibold text-cyan-400 mb-2">Input Methods</h4>
                      <div className="grid grid-cols-2 gap-2">
                        {selectedModel.features.generation.textToMusic && (
                          <div className="flex items-center h-8">
                            <i className={`${iconStyles.booleanTrue} mr-2 text-cyan-400`} />
                            <span className={textStyles.body}>Text to Music</span>
                          </div>
                        )}
                        {selectedModel.features.generation.audioToMusic && (
                          <div className="flex items-center h-8">
                            <i className={`${iconStyles.booleanTrue} mr-2 text-cyan-400`} />
                            <span className={textStyles.body}>Audio to Music</span>
                          </div>
                        )}
                        {selectedModel.features.generation.textToVoice && (
                          <div className="flex items-center h-8">
                            <i className={`${iconStyles.booleanTrue} mr-2 text-cyan-400`} />
                            <span className={textStyles.body}>Text to Voice</span>
                          </div>
                        )}
                      </div>
                    </div>
